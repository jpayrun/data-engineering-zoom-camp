...
DATA_FILE = 'yellow_tripdata_{{logical_date.strftime(\'%Y-%m\')}}.parquet'
...
with DAG(
    dag_id="data_ingestion_gcp_yellow_taxi_dag",
    start_date=dt.datetime(2019, 1, 1),
    ...
    local_to_gcs_task = PythonOperator(
        ...
        op_kwargs={
            "bucket": BUCKET,
            "object_name": f"raw/yellow_taxi/{DATA_FILE}",
    ...
    bigquery_external_table_task = BigQueryCreateExternalTableOperator(
        ...
        table_resource={
            "tableReference": {
                ...
                "tableId": "yellow_tripdata_{{logical_date.strftime(\'%Y-%m\')}}",
            },
            "externalDataConfiguration": {
                "sourceFormat": "PARQUET",
                "sourceUris": [f"gs://{BUCKET}/raw/yellow_taxi/{DATA_FILE}"],
            },
        },
    )
